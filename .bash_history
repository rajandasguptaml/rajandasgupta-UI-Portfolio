...
if [ -z "$JAVA_HOME" -a -z "$JRE_HOME" ]; then   if [ "`uname`" = "Darwin" ]; then
    if [ -x '/usr/libexec/java_home' ] ; then       export JAVA_HOME=`/usr/libexec/java_home`
    elif [ -d "/System/Library/Frameworks/JavaVM.framework/Versions/CurrentJDK/Home" ]; then       export JAVA_HOME="/System/Library/Frameworks/JavaVM.framework/Versions/CurrentJDK/Home";     fi;   else     JAVA_PATH=`which java 2>/dev/null`;     if [ "x$JAVA_PATH" != "x" ]; then       JAVA_PATH=`dirname "$JAVA_PATH" 2>/dev/null`;       JRE_HOME=`dirname "$JAVA_PATH" 2>/dev/null`;     fi;     if [ "x$JRE_HOME" = "x" ]; then
      if [ -x /usr/bin/java ]; then         JRE_HOME=/usr;       fi;     fi;   fi;   if [ -z "$JAVA_HOME" -a -z "$JRE_HOME" ]; then     echo "Neither the JAVA_HOME nor the JRE_HOME environment variable is defined";     echo "At least one of these environment variable is needed to run this program";     exit 1;   fi; fi
...
git init
git init
git clone https://github.com/rajandasguptaml/rajandasguptaml.github.io.git
git init
git add
git add .
git commit -m "rajandasgupta
git commit -m "rajandasgupta"
...
git clone
[200~io.git~
gh repo clone rajandasguptaml/rajandasguptaml.github.io
